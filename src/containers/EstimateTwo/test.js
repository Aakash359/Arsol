...
class EstimateTwoScreen extends PureComponent {
  ...
  updateList(){
    ...
    var half_gst,full_gst,full_amt
      for(var i=0;i<newArray.length;i++){
        
     
        half_gst=reg_state == customer_state? 
        sgst_rs + Math.round( parseFloat(newArray[i].item_gstamount)/ 2): 0
       
        full_gst=reg_state != customer_state? 
        igst_rs + Math.round( parseFloat(newArray[i].item_gstamount)): 0
      
        full_amt=sub_total + parseFloat(newArray[i].item_amt)
